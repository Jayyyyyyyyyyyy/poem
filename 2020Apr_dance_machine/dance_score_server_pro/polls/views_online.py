...
teachers_path = './data/output_json_teacher'
teacher_frame_dict = laod_teacher_keypoints(teachers_path)
teachers_path_flip = './data/output_json_teacher_flip'
...
def stage2_flip(img, tmp_port, teacher_time, uid, timestamp):
    ...
    if obj['code'] == 2:
        ...
        teacher_keypoint_flip = teacher_frame_dict_flip[teacher_time]

        s_org, avg_org = my_score(user_keypoint, teacher_keypoint, teacher_time)
        logging.info('org: score:{}, avg:{}'.format(s_org, avg_org))
        s_flip, avg_flip = my_score(user_keypoint, teacher_keypoint_flip, teacher_time)
        logging.info('flip: score:{}, avg:{}'.format(s_flip, avg_flip))
        s, avg, flag_from = max(zip([s_org, s_flip],[avg_org, avg_flip], ['org', 'flip']), key=lambda x: x[0])
        ...
        new_tmp_res= {'score':s, 'avg':avg, 'frame_num':teacher_time, 'timestamp':timestamp, 'user_keypoints':list(np.array(user_keypoint)[[1,3,4,6,7]]), 'teacher_keypoints':list(np.array(teacher_keypoint)[[1,3,4,6,7]])}
        distance = measure_distance(user_keypoint)
        result = {'score': s, 'distance': distance}
        result['code'] = 1
        result['uid'] = uid
        result['upload_time'] = timestamp
    return result, user_keypoint, teacher_keypoint, temp_res, new_tmp_res
...
def stage2(img,tmp_port, teacher_time, uid, timestamp):
    ...
    if obj['code'] == 2:
        ...
        user_keypoint = obj['keypoints'][0]
        teacher_keypoint = teacher_frame_dict[teacher_time]
        s, avg = my_score(user_keypoint, teacher_keypoint, teacher_time)
        temp_res = '###frame_result####\nscore:{}, avg:{}, frame_num:{}, timestamp:{}\nuser_keypoints:{}\nteacher_keypoints:{}'.format(s, avg, teacher_time, timestamp,list(np.array(user_keypoint)[[1,3,4,6,7]]), list(np.array(teacher_keypoint)[[1,3,4,6,7]]))
        logging.info(temp_res)
        new_tmp_res= {'score':s, 'avg':avg, 'frame_num':teacher_time, 'timestamp':timestamp, 'user_keypoints':list(np.array(user_keypoint)[[1,3,4,6,7]]), 'teacher_keypoints':list(np.array(teacher_keypoint)[[1,3,4,6,7]])}
        distance = measure_distance(user_keypoint)
        result = {'score': s, 'distance': distance}
    ...
    try:
        ...
        if flag == 1:
            ...
            if teacher_time in flip_frames:
                result, user_keypoint, teacher_keypoint, temp_res, new_tmp_res = stage2_flip(img, tmp_port, teacher_time, uid, timestamp)
            else:
                result, user_keypoint, teacher_keypoint, temp_res, new_tmp_res = stage2(img, tmp_port, teacher_time, uid, timestamp)
            ...
            try:
                if result['distance'] == -1:
                    ...
                    kafka_res['img'] = img
                    kafka_res['details'] = new_tmp_res
                    producer.sendjsondata(kafka_res)
                else:
                    pass
            except:
                pass
            ...
            # if user_keypoint is not None and teacher_keypoint is not None:
            #     path_parent = './tmp/{}'.format(uid)
            #     if not os.path.exists(path_parent):
            #         os.mkdir(path_parent)
            ...
            #     outfile = "{}/teacher{}.jpg".format(path_son,teacher_time)
